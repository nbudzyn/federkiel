...
/**
 * Eine Wortform (syntaktisches Wort, Flexionsform), d.h. ein Wort in einer Form, wie es im Satz
 * erscheinen könnte einschließlich seiner (grammatischen) Merkmale.
 * <p>
 * It is important to understand that objects of this class are value objects! There can be
 * arbitrarily many objects that "mean the same word form".
 *
 * @author nbudzyn 2009
 */
@Immutable
...
public class Wortform implements IWordForm {
  ...
  /**
   * Die (grammatischen) Merkmale, jeweils mit Name und Wert - EINSCHLIESSLICH der (grammatischen)
   * Merkmale des Lexems!
   */
  ...
  /**
   * Erzeugt eine neue Wortform, die der Nennform eines Lexems entspricht, <i>ohne</i>
   * Wortform-spezifische (grammatische) Merkmale.
   */
  ...
  /**
   * Erzeugt eine neue Wortform <i>ohne</i> Wortform-spezifische (grammatische) Merkmale.
   *
  ...
  /**
   * Erzeugt eine neue Wortform <i>mit</i> Wortform-spezifischen (grammatischen) Merkmalen.
   *
   * @param string must not be empty
   */
  public Wortform(final ILexeme lexeme, final String pos, final String string,
      final FeatureStructure specificFeatures, final ISemantics semantics) {
   ...
   * @param additionalSpecificFeatures additional specific features
   */
  private Wortform(final Wortform original, final String string,
      final FeatureStructure additionalSpecificFeatures) {
  ...
  public static IWordForm[] unflektMitTyp(final ILexemeType lexemeType, final String pos,
    ...
    final FeatureStructure features =
        FeatureStructure.fromStringValues(ImmutableMap.of("typ", typ));
  ...
  /**
   * Erzeugt zu dieser Wortform - die KLEINgeschrieben sein muss! - zwei Wortformen - eine
   * kleingeschriebene für das Vorkommen im Satz (i.A.) und eine Großgeschriebene für das Vorkommen
   * am Satzbeginn.
   *
