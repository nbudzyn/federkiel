...
/**
 * Eine Wortform (syntaktisches Wort, Flexionsform), d.h. ein Wort in
 * einer Form, wie es im Satz erscheinen könnte einschließlich seiner
 * (grammatischen) Merkmale.<p>
 * It is important to understand that objects of this class are
 * value objects! There can be arbitrarily many objects that
 * "mean the same word form".
 *
 * @author nbudzyn 2009
 */
@Immutable
...
public class Wortform implements IWordForm {
	private static final String IM_SATZ_MGL_KEY = "imSatzMgl";

	private static final String AM_SATZANFANG_MGL_KEY = "amSatzanfangMgl";
	 ...
	 * EINSCHLIESSLICH der (grammatischen) Merkmale des Lexems!
	 */
	...
	/**
	 * Erzeugt eine neue Wortform <i>ohne</i> Wortform-spezifische
	 * (grammatische) Merkmale.
	 * @param string
	 *            must not be empty
	 */
	public Wortform(final ILexeme lexeme, final String pos,
			final String string, final ISemantics semantics) {
		this.lexeme = lexeme;
		this.string = string;
		this.pos = pos;
	...
	/**
	 * Erzeugt eine neue Wortform
	 * <i>mit</i> Wortform-spezifischen (grammatischen) Merkmalen.
	 * @param string must not be empty
	 */
	public Wortform(final ILexeme lexeme, final String pos,
			final String string, final FeatureStructure specificFeatures, final ISemantics semantics) {
		this(lexeme, pos, string, specificFeatures, semantics, true);
	}

	/**
	 * Creates a new word form as a copy from the original.
	 * @param additionalSpecificFeatures additional specific features
	 */
	...
	private Wortform(final Wortform original, final String string, final FeatureStructure additionalSpecificFeatures) {
		this(original.lexeme, original.pos,
				string,
				FeatureStructure.disjunctUnion(
						original.features, // lexeme features are already included!
						additionalSpecificFeatures),
				original.semantics, false); // (lexeme features are already
										// included!)
	}
	 ...
	 * @param string must not be empty
	 */
	private Wortform(final ILexeme lexeme, final String pos,
			final String string, final FeatureStructure features,
			final ISemantics semantics, final boolean addFeaturesFromLexeme) {
		this.lexeme = lexeme;
		this.pos = pos;
		this.string = string;
		this.features =
				addFeaturesFromLexeme ?
				FeatureStructure.disjunctUnion(this.lexeme.getFeatures(),
								features) :
							features;
		this.semantics = semantics;
	}
	 ...
	 * @param string must not be empty
	 */
	public static Wortform lexemeFeaturesAlreadyIntegrated(final ILexeme lexeme,
			final String pos,
			final String string,
	 ...
	 */
	public static IWordForm[] unflektMitTyp (
			final ILexemeType lexemeType, final String pos, final String typ,
		...
		final FeatureStructure features =
			FeatureStructure.fromStringValues(
					ImmutableMap.of("typ", typ));

		for (int i = 0; i < strings.length; i++) {
			final Lexeme lexeme =
					new Lexeme(lexemeType, strings[i], features);
	...
	public Collection<IWordForm> expandToUpperLowerCaseForms(
		...
		if (Character.isLowerCase(getString().codePointAt(0))) {
			...
			return expandToUpperLowerCaseFormsForLowerCase(
	 ...
	 * Wortformen - eine kleingeschriebene für das Vorkommen im Satz (i.A.) und
	 * eine Großgeschriebene für das Vorkommen am Satzbeginn.
	 *
	 * @param alsoAllowLowerCaseAtSentenceStart
	 *            ob die kleingeschriebene Wortform auch am Satzbeginn stehen
	 *            darf (wie z.B. bei "von Papen sagte zu.")
	 */
	private Collection<IWordForm> expandToUpperLowerCaseFormsForLowerCase(
			final boolean alsoAllowLowerCaseAtSentenceStart,
			final Locale locale) {
		return ImmutableList.<IWordForm>of(
				new Wortform(
						this,
						StringUtil.capitalize(string, locale),
						buildCaseFeatures(true, false)),
				// am Satzbeginn
				new Wortform(
						this,
						buildCaseFeatures(
								alsoAllowLowerCaseAtSentenceStart, true))); // im
																			// Satz
	}

	private Collection<IWordForm> addUpperCaseFeatures() {
		return ImmutableList.<IWordForm>of(
				new Wortform(this, buildCaseFeatures(true, true)));
	}
	...
	/*
	public boolean featuresEqual(final Wortform other) {
