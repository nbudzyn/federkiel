...
final class ObjektTyp extends AbstractErgaenzungsOderAngabenTyp {
  ...
  private final Kasus kasus;
	 ...
	 * ("Ich habe ES gut.") - <code>false</code>, falls es sich NICHT UM EINEN
	 * PSEUDOAKTANTEN HANDELN DARF ("Ich liebe ES.", "Ich öffne DIE TUER.").
	 */
	private final boolean pseudoaktantEs;

	/**
	 * <code>true</code>, falls es sich um ein reflexives Verb handelt (er freut
	 * SICH) - <code>false</code>, falls es sowohl nurReflexivesObjektErlaubt
	 * wie auch irreflexiv verwendet werden kann
	 */
	private final boolean nurReflexivesObjektErlaubt;

	/**
	 * Minimale Anzahl an Objekten dieses Typs (normalerweise 1 - in seltenen
	 * Fällen 0)
	 */
	private final int minFillings;

	/**
	 * Maximale Anzahl an Objekten dieses Typs (normalerweise 1 - in seltenen
	 * Fällen 2)
	 */
	...
	/**
	 * Slot für ein "normales" Objekt (nicht ausschließlich reflexiv, kein
	 * Pseudoaktant, genau ein Vorkommen)
	 */
  ObjektTyp(final String slotName, final Kasus kasus) {
	...
	/**
	 * @param minFillings
	 *            Minimale Anzahl an Objekten dieses Typs (normalerweise 1 - in
	 *            seltenen Fällen 0
	 * @param maxFillings
	 *            Maximale Anzahl an Objekten dieses Typs (normalerweise 1 - in
	 *            seltenen Fällen 2)
	 */
  ObjektTyp(final String slotName, final Kasus kasus,
  ...
  public RoleFrameSlot buildSlot(final @Nullable String person,
      ...
      final @Nullable String hoeflichkeitsformDesSubjekts) {
					...
					SlotRequirements
							.of(
									...
									buildFeatureConditionForN_PRONOMEN_PHR_REIHUNGEinzelnesReflexivGebrauchtesPronomen(
											person, genusDesSubjekts, numerusDesSubjekts,
		...
		final SlotRequirements reqsAlternativeNichtReinReflexiv =
				SlotRequirements
						.of(
								"N_PRONOMEN_PHR_REIHUNG",
								buildFeatureConditionForN_PRONOMEN_PHR_REIHUNGNichtReinReflexiv(
										person, genusDesSubjekts, numerusDesSubjekts,
										hoeflichkeitsformDesSubjekts));
		return RoleFrameSlot.of(slotName,
				1, // minFillings
	...
	/**
	 * @return Eine Bedingung für eine N_PRONOMEN_PHR_REIHUNG, die sicherstellt,
	 *         dass es sich um ein einzelnes reflexiv gebrauchtes Pronomen
	 *         handelt: <i>mich</i>, <i>sich</i> - aber nicht <i>sogar mich</i>
	 *         oder <i>sich und dich</i>.
	 */
	private IFormula<FeatureAssignment> buildFeatureConditionForN_PRONOMEN_PHR_REIHUNGEinzelnesReflexivGebrauchtesPronomen(
      ...
      final Numerus numerusDesSubjekts,
        ...
        .featureEqualsExplicitValue("kasus",
		...
		featureReqs
				.addAll(buildReinReflexivConditions(person, genusDesSubjekts, numerusDesSubjekts,
						hoeflichkeitsformDesSubjekts));

		// Er freut sich, aber nicht *er freut ihn oder
		// *er freut sich selbst.
		featureReqs.add(ThreeStateFeatureEqualityFormula
				.featureEqualsExplicitValue(GermanUtil.EINZELNES_REFLEXIVPRONOMEN,
						StringFeatureLogicUtil.TRUE));
	...
	/**
	 * @return Eine Bedingung für eine N_PRONOMEN_PHR_REIHUNG, die sicherstellt,
	 *         dass alle Reflexivpronomen (sofern in diesem Objekt überhaupt
	 *         welche vorkommen, z.B. in "sich" oder in "mich und sich selbst")
	 *         Person, Numerus (Sg), Genus (z.B. m) und Höflichkeitsform (ihrer
	 *         vs. Ihrer) mit dem Subjekt übereinstimmen.
	 */
  ...
  private List<IFormula<FeatureAssignment>> buildReinReflexivConditions(
      final String person, final Genus genusDesSubjekts,
      final Numerus numerusDesSubjekts,
			final String hoeflichkeitsformDesSubjekts) {
		return buildReflexivCondition(person, genusDesSubjekts,
				numerusDesSubjekts, hoeflichkeitsformDesSubjekts,
				"person", "genus", "numerus", "hoeflichkeitsform");
	}

	/**
	 * @return eine Bedingung, die sicherstellt, dass das Reflexivpronomen soll
	 *         in Person (3!), Numerus (Sg), Genus (z.B. m) und Höflichkeitsform
	 *         (ihrer vs. Ihrer) mit dem Subjekt übereinstimmen!
	 */
	private List<IFormula<FeatureAssignment>> buildReflexivConditionsNichtUnbedingtRein(
      ...
      final Numerus numerusDesSubjekts,
		...
		// des Subjekts sind.
		return buildReflexivCondition(person, genusDesSubjekts,
				numerusDesSubjekts, hoeflichkeitsformDesSubjekts, "personDesReflexivenPronomens",
				"genusDesReflexivenPronomens", "numerusDesReflexivenPronomens",
				"hoeflichkeitsformDesReflexivenPronomens");
	}

  private List<IFormula<FeatureAssignment>> buildReflexivCondition(
      final String person, final Genus genusDesSubjekts,
      final Numerus numerusDesSubjekts,
			final String hoeflichkeitsformDesSubjekts,
			final String personMerkmalFuerReflexivbedingung,
		...
		if (UnspecifiedFeatureValue.notNullAndNotUnspecified(person)) {
			...
			featureReqs.add(ThreeStateFeatureEqualityFormula
					.featureEqualsExplicitValue(personMerkmalFuerReflexivbedingung, person));
		}
    if (genusDesSubjekts != null) {
			// Er freut sich /m, aber nicht *Er freut sich /f.
			featureReqs.add(ThreeStateFeatureEqualityFormula
					.featureEqualsExplicitValue(genusMerkmalFuerReflexivbedingung,
              FeatureStringConverter.toFeatureString(genusDesSubjekts)));
		}
    if (numerusDesSubjekts != null) {
			// Ich freue mich, aber nicht *Ich freue uns.
			featureReqs.add(ThreeStateFeatureEqualityFormula
					.featureEqualsExplicitValue(numerusMerkmalFuerReflexivbedingung,
		...
		if (UnspecifiedFeatureValue.notNullAndNotUnspecified(hoeflichkeitsformDesSubjekts)) {
			...
			featureReqs.add(ThreeStateFeatureEqualityFormula
					.featureEqualsExplicitValue(hoeflichkeitsformMerkmalFuerReflexivbedingung,
							hoeflichkeitsformDesSubjekts));
		}
