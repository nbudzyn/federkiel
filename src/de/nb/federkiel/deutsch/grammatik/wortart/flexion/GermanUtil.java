...
public final class GermanUtil {
	...
	public static final String LEXEM_FLEKTIERBAR_KEY =
			"lexemFlektierbar";
	/**
	 * für Adjektive: Frankfurter, neunziger
	 */
	public static final String ABLEITUNG_AUF_ER_KEY =
			"ableitungAufEr";
  public static final String GENITIV_SICHTBAR_DURCH_S_KEY =
			"genitivSichtbarDurchS";
  public static final String GENITIV_SICHTBAR_DURCH_R_KEY =
			"genitivSichtbarDurchR";
	protected static final String ERLAUBT_NACHGESTELLTES_SCHWACH_FLEKTIERTES_ADJEKTIV =
	 ...
	 * <li>bedauern -> bedaur (ich bedaure) (Duden 620 fordert für eine
	 * e-Tilgung bei Verben, dass -er auf einen Vokal folgt. Nach meinem
	 * Sprachgefühl ist rudern -> rudr (ich rudre) ebenfalls möglich.)
	 * </ul>
	 * </ul>
	 *
	...
	public static String tilgeEAusStammWennMoeglich(final String stamm) {
		...
		if (StringUtil
				.endsWith(stamm, "eel", "eer", "een", "iel", "ier", "ien")) {
			return null;
		}
	...
	protected static boolean endetAufBetontenVollvokal(final String wortform) {
		...
		if (StringUtil.endsWith(wortform,
				TYPISCHE_VERSCHRIFTUNGEN_LANGER_VOKALE
				)) {
			return true;
		}
		...
		if (StringUtil.endsWith(wortform, DIPHTHONGE
			...
			log.warn("Wortbildung unsicher (betonter Diphtong am Ende?): "
					+ wortform);
			return true;
		}
		...
		log.warn("Wortbildung unsicher (unbetonter Vollvokal am Ende?): "
				+ wortform);

		return false; // IDEA: Daumen drücken!
	}

	public static boolean endetAufObstruentPlusMOderN(
			final String string) {
		if (string.isEmpty()) {
			return false;
		}
	...
	private static boolean endetAufObstruent(final String string) {
		...
		if (StringUtil.endsWith(string, "ah", "eh", "ih", "oh",
				"uh", "äh", "üh", "öh", "yh")) {
			// Dehnungs-h!
		...
		return StringUtil.endsWith(string, "b", "c", "d", "f", "g",
				"h", // z.B. ch, wie in "reCH(net)"; oder sch
				"j", // (vermute ich zumindest)
				"k", "p", "q", "t", "v", "w");
	}

	public static boolean endetAufSLaut(final String wortform) {
		if (StringUtil.endsWith(wortform, "s", "ß", "x", "z",
				"ce")) { // z.B. Interface
			return true;
		}
  ...
  public static boolean endetAufVollvokal(final String wort) {
	...
	/**
	 * Heuristik - ermittelt, ob dieses Wort mit or endet und versucht zu
	 * ermitteln, ob es sich dabei wohl um eine lateinische Endung handelt
	 */
  public static boolean endetAufLateinischemOr(final String wortform) {
		...
		if (StringUtil.endsWith(wortform, "cor", "hor", "oor", "qor", "ßor",
				"wor", "zor")) {
			return false;
		}
	...
	/**
	 * @return <code>null</code>, falls sich diese Methode nicht entscheiden
	 *         kann
	 */
  public static Boolean endetUnbetontMitSilbenreimEELEr(
			final String wortform) {
	...
	/**
	 * Dies ist eine Heuristik - die im Zweifel eher mal sagt:
	 * "Nein, endet NICHT so"!
	 */
  public static boolean endetUnbetontMitSilbenreimElEnEndEmEr(
			final String wortform) {
		// Silbenreim: Alles was in der (letzten) Silbe nach dem Anfangsrand
		// (Onset) steht, -> Duden 26
		// Längere Silben ausschließen
	 ...
	 *         gemäß Duden 494 eine e-Tilgung in den <i>Suffixen</i> <i>en</i>
	 *         und <i>em</i> erlaubt (wie in <i>dunkelm</i> - statt *dunkelem).
	 *         <p>
	 *         Dieser Fall ist nicht zu verwechsln mit eine e-Tilgung im Stamm
	 *         wie in <i>dunklem</i>!
	 *         <p>
	 *         Der Stamm endet <i>nicht</i> auf <i>e</i>! (Also <i>müd</i>,
	 *         nicht <i>müde</i>.)
	 */
	protected static boolean erlaubtAdjektivischeETilgungBeiSuffixEnUndEm(
			final String stamm) {
  ...
  public static String ggfUmlauten(final String wortform,
		...
		for (int i = wortform.length() - 1; i >= 0; i--) {
			...
			if (i > 0) {
				...
				if (isVokal(vokalDavor)) {
					...
					// "eu"?
					if ((vokalDavor == 'e' && vokal == 'u') ||
							(vokalDavor == 'E' && vokal == 'u')) {
	...
	public final static KasusInfo guessGenitivInfoPl(
	...
	/**
	 * Dies ist etwas heuristisch. Fälle wie <i>Interface</i> werden nicht
	 * erkannt.
	 *
	 * @param ob
  ...
  public static boolean isLateinADeklination(final String wortform) {
	 ...
	 */
  public static boolean istFremdwortUndEndetAufBetontenVollvokal(
			final String word,
	...
	/*
	public final static boolean enthaeltWortform(
			final Collection<? extends IWordForm> wordforms,
			final String wortform) {
		for (final IWordForm element : wordforms) {
			if (element.getString().equals(wortform)) {
				return true;
			}
		}
