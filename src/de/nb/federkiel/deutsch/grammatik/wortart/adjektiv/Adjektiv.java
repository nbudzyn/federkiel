...
public class Adjektiv {
  ...
  /**
   * Die Substantivierung des Adjektivs. Bei <code>null</code> wird die Substantivierung geraten.
   */
   ...
   * Erzeugt ein Adjektiv.
   * <p>
   * Stamm und unflektierte Form kˆnnen in Einzelf‰llen voneinander abweichen, etwa <i>hoh</i> vs.
   * <i>hoch</i>, <i>ander</i> vs. <i>anders</i>.
   * <p>
   * Die fehlenden Angaben werden durch das System geraten.
   */
  ...
  private static AdjektivFlexionsklasse rateFlexionsklasse(final String stamm, final String unflektiert) {
    ...
    if (StringUtil.endsWith(stamm, "nuller", "zehner", "zwanziger", "dreiﬂiger", "vierziger",
            "f¸nfziger", "sechziger", "siebziger", "achtziger", "neunziger", "hunderter",
            "tausender")) {
      return AdjektivFlexionsklasse.ABLEITUNG_AUF_ER;
    }

    if (StringUtil.endsWith(stamm, "zwei", "drei", "vier", "f¸nf", "sechs", "sieben", "acht",
        "neun", "zehn", "elf", "zwˆlf", "zehn", "zwanzig", "dreiﬂig", "vierzig", "f¸nfzig",
   ...
   */
  public Adjektiv(final String stamm, final String unflektiert,
      final AdjektivFlexionsklasse flexionsklasse, final @Nullable Substantiv substantivierung) {
    this.stamm = stamm;
  ...
  /**
   * Schwache Flexion: der groﬂe Krieger die gute Frau das weiﬂe Einhorn des groﬂen Kriegers der
   * guten Frau des weiﬂen Einhorns dem groﬂen Krieger der guten Frau dem weiﬂen Einhorn den groﬂen
   * Krieger die gute Frau das weiﬂe Einhorn die groﬂen Krieger die guten Frauen die weiﬂen
   * Einhˆrner der groﬂen Krieger der guten Frauen der weiﬂen Einhˆrner den groﬂen Kriegern den
   * guten Frauen den weiﬂen Einhˆrnern die groﬂen Krieger die guten Frauen die weiﬂen Einhˆrner
   *
   * Starke Flexion: ein groﬂer Krieger eine gute Frau ein weiﬂes Einhorn eines groﬂen Kriegers
   * einer guten Frau eines weiﬂen Einhorns einem groﬂen Krieger einer guten Frau einem weiﬂen
   * Einhorn einen groﬂen Krieger eine gute Frau ein weiﬂes Einhorn groﬂe Krieger gute Frauen weiﬂe
   * Einhˆrner groﬂer Krieger guter Frauen weiﬂer Einhˆrner groﬂen Kriegern guten Frauen weiﬂen
  ...
  public String getWortform(final Flexionstyp inflexionType, final Kasus kasus,
    ...
    if (inflexionType == SCHWACHE_FLEXION) {
      ...
      // TODO Auﬂerdem an die richtige Stelle verschieben, dass es auch im AdjektivFlektierer
      // richtig verwendet wird.
      // Dort gibt es schon ein unflektiertPl(), das man aber anpassen muss (komparation...)
