...
/**
 * A term that takes several role frame terms and builds a collection from them.<p>
 *
 * @author nbudzyn 2009
 */
@Immutable
...
public class RoleFrameCollectionTerm implements ITerm<RoleFrameCollection, FeatureAssignment> {
	 ...
	 */
	final private static WeakCache<RoleFrameCollectionTerm> cache =
			new WeakCache<>();
	...
	private RoleFrameCollectionTerm(
			final ITerm<RoleFrame, FeatureAssignment>... roleFrameTerms) {
		super();
	 ...
	 */
	public RoleFrameCollectionTerm union(
			final RoleFrameCollectionTerm other) {
		return of(ImmutableSet.<ITerm<RoleFrame, FeatureAssignment>> builder().
				addAll(roleFrameTerms).
				addAll(other.roleFrameTerms).build());
	}
	...
	throws UnassignedVariableException {
		final ImmutableSet.Builder<RoleFrame> roleFrameValues =
	...
	public int compareTo(final ITerm<? extends Object, ? extends IAssignment> o) {
		...
		return CollectionUtil.compareCollections(
				roleFrameTerms, other.roleFrameTerms);
	}
