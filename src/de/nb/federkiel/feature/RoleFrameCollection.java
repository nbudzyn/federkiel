...
/**
 * A Collection of role frames. That collection might contain several role
 * frames - corresponding to several parts of the sentence (these are <i>not</i>
 * alternatives!).
 * <p>
...
public class RoleFrameCollection implements IFeatureValue, Iterable<RoleFrame>, IFillingUsageRestrictor {
	...
	/**
	 * Builds the union of the two role frame collections - or returns
	 * <code>null</code>, iff the two role frame collections do contain role frames
	 * which have <i>the same filling</i> filled into <i>slots with different
	 * names</i>.
	 */
	...
	protected Plurival<RoleFrameCollection> merge(final RoleFrameCollection other) {
		 ...
		 *
		 * Vielleicht ist der Haken aber auch: Adam belegt einen Slot, der in der
		 * anderen Role Frame Collection schon belegt ist?? (Das geht gar nicht mehr
		 * oder?) Adam muss in allen Rollen-Rahmen der Collection dieselbe Rolle
		...
		// erwartet,
		// jede ALTERNATIVE enthält dann wieder eine RoleFrameCollection mit allen
		// merge-Paaren...
		return Plurival.of(ImmutableList.copyOf(resAlternatives));
	}

	/**
	 * Builds a new role frame collection by using this as the base for filling an
	 * ellipse role frame collection.
	 * <p>
	 ...
	 * some feature restrictions. For example, a feature restriction could say, that
	 ...
	 * collection's role frames, and if there is more than one, the system could
	 * tell the parser, that there is no use working on this role frame collection
	 * any more.
	 * <p>
	 * Effectively, this method does a merge (of this role frame collection and the
	 ...
	 *         <ul>
	 *         <li>This role frame collection contains a role frame, that does not
	 *         have any free fillings at all. So the restrictions will not be
	 *         checked.
	 *         <li>Each role frame in this role frame collection confirms to the
			...
			// If the role frame collection contains only one role frame
				...
				for (final RoleFrameCollection oldMergeAlternative : oldMergeAlternatives) {
					...
					if (roleFrameCount == size() - 1) {
						...
						// alternative
						if (myRoleFrame.canMerge(restriction, oldMergeAlternative)) {
							return true;
						}
	...
	/*
	 * /** Builds a new role frame collection by merging this one with another one.
	 * <p> Merging two <i>role frames</i> means adding slots and filling slots with
	 * free fillings.
	 *
	 * public IPlurival<RoleFrameCollection> mergeNew(final RoleFrameCollection
	 * other) { Plurival<RoleFrameCollection> res = new
	 * Plurival<RoleFrameCollection>(); res.add(new RoleFrameCollection()); // one
	 * result: empty role frame Collection (for a start)
	 *
	 * /* Adam war Komponist und Dichter. Adam war Komponist und heiratete Eva.
	 * Peter bzw. Frank waren Komponist heirateten Eva.
	 *
	 * Das Problem ist eine Ebene höher: "Adam" wird in einem Rollen-Rahmen der Role
	 * Frame Collection als Subjekt und zugleich im anderen als Prädikatsnomen
	 * belegt -- das darf nicht sein!!! Vielleicht ist der Haken auch: Adam belegt
	 * einen Slot, der in der anderen Role Frame Collection schon belegt ist! Adam
	 * muss in allen Rollen-Rahmen der Collection dieselbe Rolle ausfüllen (oder gar
	 * keine!)
	 *
	 * Das freeFilling muss also gegen die gleichnamigen Slots ALLER ROLE FRAMES
	 * geprüft werden - nicht nur gegen einen! Das gilt natürlich SPIEGELBILDLICH!
	 * Das Merging muss also etwa so passieren wie beim Role-Frame-Merge, aber auf
	 * Ebene der Collections, wobei die GLEICHNAMIGEN SLOTS beim FILLING immer
	 * GEMEINSAM betrachtet werden müssen. Oder aber die GLEICHEN FILLINGS (gleiche
	 * von-bis-Wortfolgen) müssen gleich betrachtet werden??
	 *
	 * Bedenke aber auch: Adam und Eva waren Mann und Frau. (Wohl irrelevant. "und"
	 * ~ "bzw.")
	 */

	/*
	 * Merges the two role frames, which means: <ul> <li>free fillings are added
	 ...
	 * there are NO slots at all!) </ul> <p> The free fillings of this role frame
	 * collection and of the other role frame collection MUST BE DISJOINT (resulting
	 * from different parts of the input, for example)! <p> In some cases, merging
	 * is not possible (not all free fillings can be consumed by the slots, e.g.),
	 * in other cases there are several possibilities for a merge (filling A fills
	 * slot X, filling B fills Slot Y; or the other way round). So, the merge result
	 * is a COLLECTION, containing all possible (alternative) merges. <p> If the
	 * merge is not possible (due to a all-has-to-be-consumed-condition, e.g.), the
	 * result will be empty. <p> The two role frames MUST NOT share the same role
	 * frame name!<p>
	 *
	 *
	 ...
	 *
	 *
	 * for (final RoleFrame myRoleFrame : this.roleFrames) { for (final RoleFrame
	 * otherRoleFrame : other.roleFrames) {
	 *
	 ...
	 * // ich ja gar nicht die Fillings einfügen.... // Statt
	 * slotsAlternatives.areAllEmpty() könnte man vorher schon //
	 * this.roleFramesDontHaveSlots() && other.slots.roleFramesDontHaveSlots() //
	 * prüfen... // Das würde aber irgendwie "voraussetzen", dass IMMER ALLE Role
	 * Frames einer // Role Frame Collection Slots haben - ODER GAR KEINE haben
	 * Slots... // (Das könnte wiederum sein - wenn auch die Slots in einer VERB
	 * Role Frame Collection // schon sehr verschieden sein könnten...) // Denkbar
	 * wäre auch: TYPEN von Role Frame Collections (die, die Slot-Filling //
	 * verlangen und die anderen, die nur Container für free fillings sind)
	 *
	 * if (slotUnion.isEmpty()) { // No slots at all --> free fillings stay
	 * unconsumed! possibleMerges = Plurival.singleton(new RoleFrame(slotUnion,
	 * mergedFreeFillings)); // ==> } else {
	 *
	 * // ELSE: There could be several different slot fillings. We start the // slot
	 * alternatives we already know // and then consume the free fillings, one after
	 * the other. // (ALL fillings have to be consumed - for each slot alternative,
	 * that // has any slots at all!)
	 *
	 * for (final ParseAlternatives freeFilling : mergedFreeFillings) { final
	 * Plurival<Map<String, RoleFrameSlot>> oldSlotsAlternatives =
	 * slotsAlternatives;
	 *
	 * slotsAlternatives = new Plurival<Map<String, RoleFrameSlot>>();
	 *
	 * // Iterate over all slot alternatives we already have for (final Map<String,
	 * RoleFrameSlot> oldSlots : oldSlotsAlternatives) { // iterate over all slots
	 * in this alternative for (final RoleFrameSlot slot : oldSlots.values()) {
	 * final RoleFrameSlot filledSlot = slot.addFillingIfAccepted(freeFilling); if
	 * (filledSlot != null) { // Filling was accepted -> so we have a new filling
	 * alternative
	 *
	 * final Map<String, RoleFrameSlot> newSlots = new HashMap<String,
	 * RoleFrameSlot>(oldSlots); newSlots.put(slot.getName(), filledSlot); //
	 * (override old value) slotsAlternatives.add(newSlots); } } } }
	 *
	 * // Build resulting RoleFrames from the slot filling alternatives final
	 * Plurival<RoleFrame> possibleMergesBuilder = new Plurival<RoleFrame>(); for
	 * (final Map<String, RoleFrameSlot> newSlots : slotsAlternatives) {
	 * possibleMergesBuilder.add( new RoleFrame( newSlots,
	 * Collections.<ParseAlternatives>emptySet())); // all free fillings consumed }
	 *
	 * possibleMerges = possibleMergesBuilder.build(); }
	 *
	 * if (!possibleMerges.isEmpty()) { // merging these two role frames was
	 * possible. // So we take all alternatives we already have and // add the new
	 * possible merge result(s) to each alternative! final
	 * Plurival<RoleFrameCollection> oldRes = res;
	 *
	 * res = new Plurival<RoleFrameCollection>(); for (final RoleFrameCollection
	 * oldRoleFrameCollection : oldRes) { for (final RoleFrame possibleMerge :
	 * possibleMerges) { res.add(oldRoleFrameCollection.add(possibleMerge)); } } }
	 * else { // possibleMerges is empty!! That means, that merging of these two
	 * role frames // is not at all possible! That means, that merging these // role
	 * frame COLLECTIONS is not possible EITHER! return Plurival.empty(); } } }
	 *
	 * // Achtung: NICHT DURCHEINANDERBRINGEN: Als Ergebnis werden ALTERNATIVEN
	 * erwartet, // jede ALTERNATIVE enthält dann wieder eine RoleFrameCollection
	 * mit allen merge-Paaren... return res.build(); }
	 */

	/**
	 * Necessary, when this object is used as an
	 * <code>IFillingUsageRestrictor</code> - returns the only slot name which is
	...
	public int howManyAdditionalFillingsAreAllowed(final String slotName) {
		...
		for (final RoleFrame roleFrame : roleFrames) {
			...
			if (howManyAdditionalFillingsAreAllowedForThisRoleFrameAndSlotName != -1) {
				if (res == -1) {
					res = howManyAdditionalFillingsAreAllowedForThisRoleFrameAndSlotName;
				} else {
					res = Math.min(res, howManyAdditionalFillingsAreAllowedForThisRoleFrameAndSlotName);
				}
			}
		}
	 ...
	 * final IFeatureValue otherValue) throws TooManyDifferencesException { if (!
	 * (otherValue instanceof RoleFrameCollection)) { throw new
	 * TooManyDifferencesException(
	 * "Values not equal: Other value is no role frame collection!"); }
	 *
	 * final RoleFrameCollection otherRoleFrameCollection = (RoleFrameCollection)
	 * otherValue;
	 *
	 * final int numMyRoleFrames = this.roleFrames.size(); if (numMyRoleFrames !=
	 * otherRoleFrameCollection.roleFrames.size()) { throw new
	 * TooManyDifferencesException(
	 * "Role frame collections not equal: Different size!"); }
	 *
	 * // they have the same number of role frames
	 *
	 * if (numMyRoleFrames == 0) { // they both are empty return null; // ==> fine }
	 *
	 *
	 * if (numMyRoleFrames != 1) { // I test for full equality - not sure how to
	 * make it better // TODO Have a better idea and check the role frames for //
	 * respective equality / findSinglePointOfDifference(). if
	 * (this.roleFrames.equals(otherRoleFrameCollection.roleFrames)) { return null;
	 * // ==> fine } else { throw new TooManyDifferencesException(
	 * "Role frame collections not equal: Role frames not equal, respectively."); }
	 * }
	 *
	 * // they both have exactly one role frame final RoleFrame myRoleFrame =
	 * this.roleFrames.iterator().next();
	 *
	 * final RoleFrame othersRoleFrame =
	 * otherRoleFrameCollection.roleFrames.iterator().next();
	 *
	 * return myRoleFrame.findSinglePointOfDifference(othersRoleFrame); }
	 */

	/*
	 * If we have two terms with two term bounds, what will be the resulting term
	 * bound for the term, that results from merging the two terms? <p> This method
	 * expects, that the term bounds are given as role frame collections
	 * (<code>this</code> and <code>otherBoundsGivenAsRoleFrameCollection</code>).
	 *
	 * public Plurival<RoleFrame> mergeBoundsGivenAsRoleFrameCollections( final
	 * RoleFrameCollection otherBoundsGivenAsRoleFrameCollection) { final
	 * Plurival<RoleFrameCollection> boundAlternativesAsRoleFrameCollections =
	 * merge(otherBoundsGivenAsRoleFrameCollection);
	 *
	 * return combineAlternativeBoundsGivenAsAlternativeRoleFrameCollection(
	 * boundAlternativesAsRoleFrameCollections); }
	 *
	 * private static Plurival<RoleFrame>
	 * combineAlternativeBoundsGivenAsAlternativeRoleFrameCollection( final
	 * Plurival<RoleFrameCollection> boundAlternativesAsRoleFrameCollections) {
	 * final Builder<RoleFrame> resBoundAlternatives = ImmutableList .<RoleFrame>
	 * builder();
	 *
	 * for (final RoleFrameCollection boundsAlternativeAsRoleFrameCollection :
	 * boundAlternativesAsRoleFrameCollections) { // Each role frame in the
	 * collection might bring some // bounds // with it - they must all be combined!
	 * (Remember, that // when (in a later step) any role // frame from a collection
	 * cannot be merged, the whole // collection cannot be merged!) try {
	 * resBoundAlternatives .add(boundsAlternativeAsRoleFrameCollection
	 * .combineBoundsGivenAsRoleFrameCollection()); } catch (final
	 * CannotFulfillTermException e) { // bad luck, the ristriction from this role
	 * frame // collection cannot even be combined. // Try the next alternative. } }
	 *
	 ...
	 *
	 * @throws CannotFulfillTermException iff the bounds cannot be combined
	 *
	 * public RoleFrame combineBoundsGivenAsRoleFrameCollection() throws
	 * CannotFulfillTermException { RoleFrame resBounds = null;
	 *
	 * for (final RoleFrame roleFrameFromBoundsAlternative : this) { if (resBounds
	 * == null) { resBounds = roleFrameFromBoundsAlternative; } else { final
	 * ITermBounds newResBoundsAsTermBounds = resBounds
	 * .combineBounds(roleFrameFromBoundsAlternative); // CannotFulfillTermException
	 *
	 * if (!(newResBoundsAsTermBounds instanceof RoleFrame)) { throw new
	 * IllegalStateException( "Combining to role frame bounds " +
	...
	public Plurival<IFillingInSlot> toFillingInSlot() {
		final ImmutableSet.Builder<IFillingInSlot> res = ImmutableSet.builder();

		for (final RoleFrame roleFrame : roleFrames) {
			@Nullable
			final IFillingInSlot fillingInSlot = roleFrame.toFillingInSlot();
	 ...
	 *
	 * not used Currently private Plurival<Map<String, RoleFrameSlot>>
	 * slotUnionAlternatives ( final RoleFrameCollection other) { final
	 * ImmutableList.Builder<Map<String, RoleFrameSlot>> res =
	 * ImmutableList.<Map<String,RoleFrameSlot>>builder();
	 *
	 * for (final RoleFrame myRoleFrame : this.roleFrames) { for (final RoleFrame
	 * otherRoleFrame : other.roleFrames) { res.add(RoleFrame.slotUnion(myRoleFrame,
	 * otherRoleFrame)); } }
	 *
