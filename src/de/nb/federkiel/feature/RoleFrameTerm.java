...
/**
 * A TERM that builds up a role frame, based on some slot TERMS
 * and some free filling TERMS.
 *
 * @author nbudzyn 2009
 */
@Immutable
@ThreadSafe
public class RoleFrameTerm
implements ITerm<RoleFrame, FeatureAssignment> {
	 ...
	 */
	private final ImmutableCollection<ITerm<IHomogeneousConstituentAlternatives, FeatureAssignment>>
	freeFillingTerms;
	...
	public RoleFrameTerm(final RoleFrameSlotTerm... slotTerms) {
		...
		this.slotTerms =
			Maps.<String, RoleFrameSlotTerm>uniqueIndex(
					Arrays.asList(slotTerms),
	...
	public RoleFrameTerm(
			final ImmutableMap<String, RoleFrameSlotTerm> slotTerms,
			final ImmutableCollection<ITerm<IHomogeneousConstituentAlternatives, FeatureAssignment>>
			freeFillingTerms) {
		if (CHECK_ASSERTIONS) {
	...
	throws UnassignedVariableException {
		...
		for (final ITerm<IHomogeneousConstituentAlternatives, FeatureAssignment> freeFillingTerm :
			freeFillingTerms) {
		...
		for (final Map.Entry<String, RoleFrameSlotTerm> slotMapEntry :
			slotTerms.entrySet()) {
	...
	/*
	 * As bounds for the term, this returns
	 * <ul>
	 * <li>a role frame containing all slots (they might be filled or not), or
	 * <li>all free fillings, or
	 * <li><code>null</code>
	 * </ul>
	 *
	@Override
	public BoundsForTermAndBoundsForVariable retrieveImplicitBoundsForTermAndBoundsForVariable(
			final Variable<RoleFrame, FeatureAssignment> variable,
			final FeatureAssignment assignment) {
		if (!this.slotTerms.isEmpty()) {
			// there are slot terms, so there are no free filling terms
			final ImmutableMap.Builder<String, RoleFrameSlot> slotBounds = ImmutableMap
			.<String, RoleFrameSlot> builder();

			for (final Map.Entry<String, RoleFrameSlotTerm> slotMapEntry : this.slotTerms
					.entrySet()) {
				RoleFrameSlot slotValue;
				try {
					// try to evaluate the slot filling term
					slotValue = slotMapEntry.getValue().evaluate(assignment);
				} catch (final UnassignedVariableException e) {
					// impossible -> build an empty slot
					slotValue = new RoleFrameSlot(
							slotMapEntry.getKey(),
				...
				slotBounds.put(slotMapEntry.getKey(), slotValue);
			}
			return new BoundsForTermAndBoundsForVariable(new RoleFrame(
					slotBounds.build()), null);
		}
		...
		try {
			for (final ITerm<IHomogeneousConstituentAlternatives, FeatureAssignment> freeFillingTerm : this.freeFillingTerms) {
				freeFillingsBounds.add(freeFillingTerm.evaluate(assignment));
				// UnassignedVariableException
			}

			return new BoundsForTermAndBoundsForVariable(new RoleFrame(
					freeFillingsBounds.build()), null);
		} catch (final UnassignedVariableException e) {
	...
	public int compareTo(final ITerm<? extends Object, ? extends IAssignment> o) {
		...
		final int slotTermsCompared = CollectionUtil.compareMaps(
				slotTerms, other.slotTerms);
